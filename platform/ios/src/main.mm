...
- (void)stopOrClose {
    ...
    self.captionLabel.text =
        @"PvZ2 v125 LIVE — HARD STOP requested; stopping guest at the next checkpoint…";
...
- (void)selectApkForJni {
    ...
    const PvZ2DiagnosticMode selectedMode =
        PvZ2DiagnosticMode::V125RepeatEnqueueContext;
    ...
    NSString *selectedModeName =
        selectedDescriptor != nullptr
            ...
            : @"V125_REPEAT_ENQUEUE_CONTEXT";

    [self
        appendUI:
            [NSString
                stringWithFormat:
                    @"STEP 3: select BOTH files at once: the original PvZ2 1.5.252752 APK and main.7.com.ea.game.pvz2_row.obb. v125 mode=%@. Stable v118-v124 runtime. Exact repeated PCM events are now traced to the guest execution context that performs the enqueue (audio callback vs CAkAudioThread vs main/other worker), with callback/queue provenance. Keep the crackles inside the same two frame windows, then Hard Stop.",
                    selectedModeName]];
...
- (void)documentPicker:
    ...
    [self
        appendUI:
            [NSString
                stringWithFormat:
                    @"=== PvZ2 v125 Repeat Enqueue Context started mode=%@; PID=%d ===",
                    diagnosticModeName,
                    getpid()]];
                            ...
                            if (PvZ2DiagnosticModeHasCapability(
                                ...
                                [selfRef.liveController
                                    finishRunWithMessage:
                                        [NSString
                                            stringWithFormat:
                                                @"PvZ2 v125 LIVE — HARD STOPPED after %u guest frames.\nClose to inspect the v125 runtime log.",
                                                result.draw_frames_completed]];
                            } else {
                                [selfRef.liveController
                                    finishRunWithMessage:
                                        [NSString
                                            stringWithFormat:
                                                @"PvZ2 v125 LIVE — run finished after %u guest frames.\nClose to inspect the v125 runtime log.",
                                                result.draw_frames_completed]];
                            }
                        ...
                        if (PvZ2DiagnosticModeHasCapability(
                            ...
                            if (result.hard_stop_requested) {
                                [selfRef.liveController finishRunWithMessage:
                                    @"PvZ2 v125 LIVE — HARD STOPPED.\nGuest execution was interrupted at the next Dynarmic checkpoint. Close to inspect the v125 runtime log."];
                            } else {
                                [selfRef.liveController finishRunWithMessage:
                                    [NSString stringWithFormat:
                                        @"PvZ2 v125 LIVE — guest stopped/crashed.\n%@\nClose to inspect the v125 runtime log.", message]];
                            }
